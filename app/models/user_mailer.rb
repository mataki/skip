...
class UserMailer < ActionMailer::Base
  ...
  def sent_contact(recipient, user_name, entry_url, entry_title)
    ...
    @subject    = UserMailer.base64("[#{CUSTOM_RITERAL[:abbr_app_title]}] #{user_name}さんから連絡がきています")
    @from       = @@from
    @send_on    = Time.now
    @headers    = {}
  ...
  def sent_message(recipient, link_url, message ,message_manage_url)
    @recipients = recipient
    @subject    = UserMailer.base64("[#{CUSTOM_RITERAL[:abbr_app_title]}] #{message}")
    @from       = @@from
    @send_on    = Time.now
    @headers    = {}
  ...
  def sent_signup_confirm(recipient, confirm_url)
    @recipients = recipient
    @subject    = UserMailer.base64("[#{CUSTOM_RITERAL[:abbr_app_title]}] ユーザ登録の確認メールです")
    @from       = @@from
    @send_on    = Time.now
    @headers    = {}
  ...
  def sent_apply_email_confirm(recipient, confirm_url)
    @recipients = recipient
    @subject    = UserMailer.base64("[#{CUSTOM_RITERAL[:abbr_app_title]}] メールアドレス変更の確認メールです")
    @from       = @@from
    @send_on    = Time.now
    @headers    = {}
    @body       = {:confirm_url=>confirm_url, :footer => @@footer}
...
private
  ...
  @@site_url = CUSTOM_RITERAL[:contact_url]
  @@system_mail_addr = CUSTOM_RITERAL[:contact_addr]
  @@from = UserMailer.base64(CUSTOM_RITERAL[:abbr_app_title]) + "<#{@@system_mail_addr}>"
  @@footer = "----\n#{@@system_mail_addr}\n#{@@site_url}"
