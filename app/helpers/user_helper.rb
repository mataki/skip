...
module UserHelper
  ...
  def user_tab_menu_source user
    ...
    tab_menu_source << {:label => _('Shared Files'), :options => {:controller => 'share_file', :action => 'list', :uid => user.uid, :id => 'sort_type_date', :sort_type => 'date'}} unless ShareFile.owned(user).accessible(current_user).empty?
