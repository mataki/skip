...
class MypageController < ApplicationController
  ...
  def load_rss_feed
    feeds = []
    MYPAGE_FEED_SETTINGS.each do |setting|
      feed = open(setting[:url], :proxy => ENV['PROXY_URL']){ |f| RSS::Parser.parse(f.read) }
      feed.channel.title = setting[:title] if setting[:title]
      limit = (setting[:limit]||MYPAGE_FEED_DEFAULT_LIMIT)
...
private
  ...
  def get_index_default
    ...
    Group::CATEGORY_KEYS.each do |category| # 表示順序の制御
      options = { :group_symbols => gid_by_category[category], :recent_day => 10, :per_page => 3 }
      recent_bbs << self.send("find_recent_bbs_#{category.downcase}_as_locals", options)
    end
  ...
  def find_recent_blogs_as_locals options = { :recent_day => 10, :per_page => 4 }
  ...
  Group::CATEGORY_KEYS.each do |category|
    define_method( "find_recent_bbs_#{category.downcase}_as_locals" ) do |options|
      options ||= {}
  ...
  def recent_bbs_proc category, options
    title   = "最新の掲示板のエントリ（#{Group.category_icon_name(category).last}）"
    id_name = "recent_bbs_#{category.downcase}"
    pages_obj, pages = nil, []

    find_options = {:exclude_entry_type=>'DIARY', :publication_type => 'public', :recent_day=>options[:recent_day]}
    find_options[:symbols] = options[:group_symbols] || Group.gid_by_category[category]
  ...
  def find_access_blogs_as_locals options = { :recent_day => 10, :per_page => 10 }
  ...
  # 引数：recent_day = 最近を示す日数（デフォルト10日）
  # 引数：per_page   = １ページの表示数（デフォルト5件）
  def find_questions_as_locals options = { :recent_day => 10, :per_page => 5 }
  ...
  # 引数：recent_day = 最近を示す日数（デフォルト10日）
  # 引数：per_page   = １ページの表示数（デフォルト5件）
  def find_not_reading_blogs_as_locals options = { :recent_day => 10, :per_page => 5 }
