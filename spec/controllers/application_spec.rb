...
describe ApplicationController, "#remove_message" do
  before do
    @link_url = "http://localhost:3000/user/a_user/blog?entry_id=6"
    @path = "/user/a_user/blog?entry_id=6"
    @user = user_login
    controller.stub!(:session).and_return({ :user_id => "1", :user_code => "111111"})
    controller.stub!(:request).and_return(mock("request", :protocol => "http://", :host_with_port => "localhost:3000", :relative_url_root => "", :url => @link_url, :request_uri => @path, :env => { "REQUEST_URI" => @path }))
    controller.stub!(:headers).and_return({ "Status" => "200 OK" })
  end
  describe "全体のURLが同じものがある場合" do
    before do
      Message.create!(:link_url => @link_url, :user_id => @user.id, :message_type => "COMMENT")
    end
    it "そのMessageを削除する" do
      perform_remove_message.should change(Message, :count).by(-1)
    end
  end
  describe "ホストを除いたURLが同じものがある場合" do
    before do
      Message.create!(:link_url => @path, :user_id => @user.id, :message_type => "COMMENT")
    end
    it "そのMessageを削除する" do
      perform_remove_message.should change(Message, :count).by(-1)
    end
  end
  describe "違うURLの場合" do
    before do
      Message.create!(:link_url => @path + "&hoge=fuga", :user_id => @user.id, :message_type => "COMMENT")
    end
    it "そのMessageは削除されない" do
      perform_remove_message.should_not change(Message, :count)
    end
  end
  def perform_remove_message
    lambda { controller.send(:remove_message) }
  end
end
